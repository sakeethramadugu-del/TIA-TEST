...
def migrate_user_settings_schema():
    ...
    c = conn.cursor()
    try:
        c.execute("ALTER TABLE user_settings ADD COLUMN otp_enabled INTEGER DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    conn.commit()
    conn.close()
...
def create_password_reset_token(email):
    ...
    if not user:
        return None, None
    token = hashlib.sha256(f"{user['id']}-{email}-{time.time()}-{random.random()}".encode()).hexdigest()[:12]
...
def get_user_settings(user_id):
    ...
    c = conn.cursor()
    row = c.execute(
        "SELECT theme, notifications, email_updates, otp_enabled FROM user_settings WHERE user_id = ?",
        (user_id,),
    ).fetchone()
    if row:
        settings = {
    ...
    c.execute(
        "INSERT INTO user_settings (user_id, theme, notifications, email_updates, otp_enabled) VALUES (?, ?, ?, ?, ?)",
        (user_id, "dark", 1, 0, 0),
    )
    conn.commit()
    conn.close()
    return {"theme": "dark", "notifications": True, "email_updates": False, "otp_enabled": False}

def save_user_settings(user_id, theme=None, notifications=None, email_updates=None, otp_enabled=None):
    conn = get_db_connection()
    c = conn.cursor()
    ...
    if not row:
        c.execute(
            "INSERT INTO user_settings (user_id, theme, notifications, email_updates, otp_enabled) VALUES (?, ?, ?, ?, ?)",
            (user_id, theme or "dark", int(bool(notifications)), int(bool(email_updates)), int(bool(otp_enabled)))
        )
    else:
        updates = []
        params = []
        ...
        if otp_enabled is not None:
            ...
            params.append(int(bool(otp_enabled)))
        if updates:
            params.append(user_id)
...
def login():
    if request.method == "POST":
        ...
        if settings.get("otp_enabled"):
            code = f"{random.randint(100000, 999999):06d}"
            session["pending_2fa_user_id"] = user["id"]
            session["pending_2fa_code"] = code
            session["pending_2fa_expires"] = time.time() + 300
            session["pending_2fa_username"] = user["username"]
            return render_template("twofactor.html", message="Enter the 6-digit code to continue.", preview_code=code)
...
def two_factor():
    if request.method == "POST":
        ...
        if not otp:
            return render_template("twofactor.html", error="Enter the code sent to your email.", preview_code=session.get("pending_2fa_code"))
        expires = session.get("pending_2fa_expires", 0)
        if time.time() > expires:
            session.pop("pending_2fa_user_id", None)
            session.pop("pending_2fa_code", None)
            session.pop("pending_2fa_expires", None)
            session.pop("pending_2fa_username", None)
            return render_template("twofactor.html", error="Code expired. Please log in again.")

        if otp != session.get("pending_2fa_code"):
            return render_template("twofactor.html", error="Invalid verification code.", preview_code=session.get("pending_2fa_code"))

        user = find_user_by_id(session.get("pending_2fa_user_id"))
        if not user:
            return render_template("login.html", error="Unable to verify two-factor authentication.")
    ...
    return render_template("twofactor.html", preview_code=session.get("pending_2fa_code"))
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not email:
            ...
            user, token = create_password_reset_token(email)
            if token:
                message = "A reset token was created. Use it to reset your password below."
                reset_token = token
            else:
                message = "If an account exists for that email, a reset token was generated."
    return render_template("forgot.html", message=message, error=error, reset_token=reset_token)
...
def reset_password(token):
    ...
    if request.method == "POST":
        ...
        if not password or not confirm:
            ...
            success, msg = set_user_password(user_id, password)
            if not success:
...
def update_two_factor():
    user_id = session.get("user_id")
    if session.get("is_guest") or not user_id:
        return safe_json_response({"error": "Login required for 2FA update", "status": "error"}, 401)
    data = request.get_json() or {}
    enabled = bool(data.get("enabled", False))
    try:
        save_user_settings(user_id, otp_enabled=enabled)
        return safe_json_response({"status": "success", "message": "Two-factor authentication updated", "otp_enabled": enabled})
    except Exception as e:
        return safe_json_response({"error": f"Failed to update 2FA: {str(e)}", "status": "error"}, 500)
...
def logout_all_devices():
    session.clear()
    return safe_json_response({"status": "success", "message": "Logged out of this session."})
