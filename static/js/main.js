...
function showGlobalError(message) {
    ...
    if (!banner) return;
    banner.innerText = message || "An unexpected issue occurred. Please refresh the page.";
    banner.classList.remove("hidden");
}

window.addEventListener("error", (event) => {
    console.error("Global JS error:", event.error || event.message);
    showGlobalError("Something went wrong while loading the dashboard. Refresh to continue.");
});

window.addEventListener("unhandledrejection", (event) => {
    console.error("Unhandled promise rejection:", event.reason);
    showGlobalError("A background error occurred. Refresh or try again later.");
});
...
function setMarketMood() {
    ...
    const diff = portfolioValue - state.startingCash;
    const mood = diff >= 200 ? "Bullish" : diff <= -200 ? "Cautious" : "Stable";
    moodEl.innerText = mood;
    moodEl.className = `status-pill mood-pill ${mood.toLowerCase()}`;
}

function drawPortfolioSparkline() {
    ...
    const values = [0, 15, 12, 18, 22, 19, 26, 24, 28, 31].map(v => v + Math.random() * 3);
...
function renderTrendingStocks() {
    const trendingStocks = [
        { ticker: "AAPL", name: "Apple", change: 1.8, price: 192.18 },
        { ticker: "TSLA", name: "Tesla", change: -0.9, price: 242.70 },
        { ticker: "NVDA", name: "NVIDIA", change: 2.4, price: 876.30 },
        { ticker: "SPY", name: "S&P 500", change: 0.3, price: 506.10 }
    ];
        ...
        return `
            <div class="trending-card">
                <div class="trending-header">
                    ...
                    <div class="trending-badge ${trendClass}">${stock.change >= 0 ? "+" : ""}${stock.change.toFixed(2)}%</div>
                </div>
                <div class="trending-value">${formatMoney(stock.price)}</div>
                <div class="trending-meta">Live • Tech</div>
                <button class="add-watchlist-btn" data-ticker="${stock.ticker}">+ Watchlist</button>
            </div>
        `;
    }).join("");
...
function setupDailyReward() {
    ...
    const claimed = localStorage.getItem("apexvestDailyRewardDate");
    if (claimed !== today) {
        setTimeout(() => {
...
function claimDailyReward() {
    ...
    localStorage.setItem("apexvestDailyRewardDate", new Date().toISOString().slice(0, 10));
    showToast("Daily login reward claimed! +25 XP", "success");
}

function dismissDailyReward() {
    const popup = document.getElementById("dailyRewardPopup");
    if (popup) popup.classList.add("hidden");
    localStorage.setItem("apexvestDailyRewardDate", new Date().toISOString().slice(0, 10));
}
...
function applyTheme(theme) {
    ...
    localStorage.setItem("apexvestTheme", theme);
...
function initTheme() {
    const saved = localStorage.getItem("apexvestTheme");
...
function setMarketStatus() {
    ...
    const timeLabel = document.getElementById("marketTime");
    if (pill) {
        pill.innerText = open ? "Market open" : "Market closed";
        pill.className = `status-pill ${open ? "status-open" : "status-closed"}`;
    }
    if (timeLabel) {
        timeLabel.innerText = `Updated ${now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
    }
}
...
async function performServerTrade(action) {
    ...
    if (!state.currentTicker) {
        showToast("Select a stock before trading.", "error");
        return;
    }
...
export async function loadStock(overrideTicker) {
    ...
    const ticker = (overrideTicker || tickerInput?.value.trim() || state.currentTicker || "").toUpperCase();
    if (!ticker) {
        hideLoading();
        showToast("Enter a ticker to load.", "error");
        return;
    }
    ...
    if (data.error) {
        hideLoading();
        showToast(data.error || "Unable to load stock.", "error");
        return;
    }

    state.currentTicker = data.ticker || ticker;
    ...
    if (tickerEl) tickerEl.innerText = data.ticker || state.currentTicker;
    const companyNameEl = document.getElementById("companyName");
    if (companyNameEl) companyNameEl.innerText = data.name || "Unknown company";
    const priceEl = document.getElementById("price");
    if (priceEl) priceEl.innerText = formatMoney(data.price || 0);
    const changeEl = document.getElementById("change");
    if (changeEl) changeEl.innerText = `${typeof data.change === "number" ? data.change.toFixed(2) : 0}% today`;
    const openEl = document.getElementById("open");
    if (openEl) openEl.innerText = formatMoney(data.open || 0);
    const highEl = document.getElementById("high");
    if (highEl) highEl.innerText = formatMoney(data.high || 0);
    const lowEl = document.getElementById("low");
    if (lowEl) lowEl.innerText = formatMoney(data.low || 0);
    const quoteTimeEl = document.getElementById("quoteTime");
    if (quoteTimeEl) quoteTimeEl.innerText = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
...
export async function loadQuestion() {
    ...
    const options = state.currentQuestionData?.opts || [];
    state.currentQuizOptionOrder = shuffle(options.map((text, index) => ({ text, index })));
    const questionEl = document.getElementById("question");
    if (questionEl) {
        questionEl.innerText = state.currentQuestionData?.q || "Question data unavailable";
    }
    const buttons = document.querySelectorAll(".quizButtons button");
    buttons.forEach((button, index) => {
        button.innerText = state.currentQuizOptionOrder[index]?.text || `Option ${index + 1}`;
    ...
    if (feedbackEl) {
        ...
        feedbackEl.innerText = "Choose an answer to test your knowledge.";
    }
...
window.onload = async () => {
    try {
        ...
        renderGlossary();
        renderHistory();
        const tickerInput = safeGet("tickerInput");
        if (tickerInput) {
            tickerInput.value = state.currentTicker;
        }
        ...
        showToast("Some dashboard features failed to load. Refresh to retry.", "warning");
    } finally {
        document.body.classList.add("page-ready");
        loadDailyChallenges();
        loadStreaks();
